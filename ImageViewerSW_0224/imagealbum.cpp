    ...
    : QWidget(parent), ui(new Ui::ImageAlbum)
{
    ...
    ui->Reverse->setStyleSheet("QToolButton { "
                               ...
                               "border-radius:10px;"
                               "}");
    ui->Prescription->setStyleSheet(pushButtonStyle);
    ui->EndTreatment->setStyleSheet(pushButtonStyle);
    ui->OrigImage->setStyleSheet(toolButtonStyle);
    ui->ZoomIn->setStyleSheet(toolButtonStyle);
    ui->ZoomOut->setStyleSheet(toolButtonStyle);
    ui->VReverse->setStyleSheet(toolButtonStyle);
    ui->HReverse->setStyleSheet(toolButtonStyle);
    ui->OrigImage2->setStyleSheet(toolButtonStyle);
    ui->Brush->setStyleSheet(toolButtonStyle);
    ui->RightRotate->setStyleSheet(toolButtonStyle);
    ui->LeftRotate->setStyleSheet(toolButtonStyle);
    ui->Blur->setStyleSheet(toolButtonStyle);
    ui->Lines->setStyleSheet(toolButtonStyle);
    ui->Sharpening->setStyleSheet(toolButtonStyle);
    ui->Freehand->setStyleSheet(toolButtonStyle);
    ui->Laser->setStyleSheet(toolButtonStyle);
    ui->Rectangle->setStyleSheet(toolButtonStyle);
    ui->Ellipse->setStyleSheet(toolButtonStyle);
    ui->Delete->setStyleSheet(toolButtonStyle);
    ui->Cursor->setStyleSheet(toolButtonStyle);
    ui->LengthMeasurement->setStyleSheet(toolButtonStyle);
    ui->Angle->setStyleSheet(toolButtonStyle);
    ui->Capture->setStyleSheet(toolButtonStyle);
    ui->Implant->setStyleSheet(toolButtonStyle);
...
{
    // 이미지가 선택되지 않았다면 예외처리
    if(selectImage.isNull() || ImageCheck == false){
